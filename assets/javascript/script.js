  ...
  var frequency = "";
   ...
   $(document).ready(function(){
   	...
   	dataRef.ref().on("child_added",  function(childSnapshot){
   		...
   		var trainSinceFirst = Math.floor(timeDiff/frequency);

   		// How long till the next train
   		var nextTrainArrival = ((trainSinceFirst + 1) * frequency) + firstTrainMoment;

   		// values for Minutes away and next arrival
   		if( firstTrainMoment < timeMoment ){
   			var minutesAway = nextTrainArrival - timeMoment;
   			var nextTrainArrival = moment().add(minutesAway, 'minutes').format('HH:mm');

   		}
   		else{
   			var nextTrainArrival = firstTrain
   			var minutesAway = firstTrainMoment - timeMoment;
   			// console.log(minAway);
   		};
